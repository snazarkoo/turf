import rhumbCircle from '@turf/rhumb-circle';
...
export default function lineArc(
    ...
    if (angle1 === angle2) {
        return lineString(
            rhumbCircle(center, radius, options).geometry.coordinates[0],
            properties
        );
    }
    ...
    while (alfa < arcEndDegree) {
        coordinates.push(
            rhumbDestination(center, radius, alfa, options).geometry.coordinates
        );
        i++;
    ...
    if (alfa > arcEndDegree) {
        coordinates.push(
            rhumbDestination(center, radius, arcEndDegree, options).geometry
                .coordinates
        );
    }
